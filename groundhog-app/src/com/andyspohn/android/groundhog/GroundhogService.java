...
import com.andyspohn.android.groundhog.handler.DefaultHandler;
...
public class GroundhogService extends Service {
    ...
    public class LocalBinder extends Binder {
        GroundhogService getService() {
            // Return this instance of LocalService so clients can call public methods
            return GroundhogService.this;
        }
    }
     ...
     *
     */
    public GroundhogService() {
        super();
        _handler = new android.os.Handler() {
            public void handleMessage(Message msg) {
                switch (msg.getData().getInt("state")) {
                    case __STARTED: {
                        GroundhogToast.showServiceToast(GroundhogService.this, R.string.jetty_started);
                        mNM = (NotificationManager) getSystemService(NOTIFICATION_SERVICE);
                        // The PendingIntent to launch IJetty activity if the user selects this notification
                        PendingIntent contentIntent = PendingIntent.getActivity(GroundhogService.this, 0,
                                new Intent(GroundhogService.this, Groundhog.class), 0);

                        CharSequence text = getText(R.string.manage_jetty);

                        Notification notification = new Notification(R.drawable.groundhog_stat,
                                text,
                                System.currentTimeMillis());

                        notification.setLatestEventInfo(GroundhogService.this, getText(R.string.app_name),
                                text, contentIntent);

                        mNM.notify(R.string.jetty_started, notification);

                        Intent startIntent = new Intent(Groundhog.__START_ACTION);
                    ...
                    case __NOT_STARTED: {
                        GroundhogToast.showServiceToast(GroundhogService.this, R.string.jetty_not_started);
                        break;
                    }
                    case __STOPPED: {
                        ...
                        GroundhogToast.showServiceToast(GroundhogService.this, R.string.jetty_stopped);
                        Intent stopIntent = new Intent(Groundhog.__STOP_ACTION);
                    ...
                    case __NOT_STOPPED: {
                        GroundhogToast.showServiceToast(GroundhogService.this, R.string.jetty_not_stopped);
                        break;
                    }
                    case __STARTING: {
                        GroundhogToast.showServiceToast(GroundhogService.this, R.string.jetty_starting);
                        break;
                    }
                    case __STOPPING: {
                        GroundhogToast.showServiceToast(GroundhogService.this, R.string.jetty_stopping);
                        break;
                    }
                }
    ...
    public void onStart(Intent intent, int startId) {
        if (server != null) {
            GroundhogToast.showServiceToast(GroundhogService.this, R.string.jetty_already_started);
            return;
        }
    ...
    public void onDestroy() {
        try {
            ...
            if (server != null) {
                ...
                GroundhogToast.showServiceToast(GroundhogService.this, R.string.jetty_not_running);
            }
        } catch (Exception e) {
            Log.e(TAG, "Error stopping jetty", e);
            GroundhogToast.showServiceToast(GroundhogService.this, R.string.jetty_not_stopped);
    ...
    protected void configureDeployers() throws Exception {
        ...
        File jettyDir = Groundhog.__JETTY_DIR;

        if (jettyDir.exists()) {
            // If the webapps dir exists, start the static webapp deployer
            if (new File(jettyDir, Groundhog.__WEBAPP_DIR).exists()) {
                staticDeployer.setWebAppDir(Groundhog.__JETTY_DIR + "/" + Groundhog.__WEBAPP_DIR);
                staticDeployer.setDefaultsDescriptor(Groundhog.__JETTY_DIR + "/" + Groundhog.__ETC_DIR + "/webdefault.xml");
                staticDeployer.setContexts(contexts);
                staticDeployer.setAttribute(CONTENT_RESOLVER_ATTRIBUTE, getContentResolver());
                staticDeployer.setAttribute(ANDROID_CONTEXT_ATTRIBUTE, (Context) GroundhogService.this);
            ...
            if (new File(jettyDir, Groundhog.__CONTEXTS_DIR).exists()) {
                contextDeployer.setScanInterval(10); // Don't eat the battery
                contextDeployer.setConfigurationDir(Groundhog.__JETTY_DIR + "/" + Groundhog.__CONTEXTS_DIR);
                contextDeployer.setAttribute(CONTENT_RESOLVER_ATTRIBUTE, getContentResolver());
                contextDeployer.setAttribute(ANDROID_CONTEXT_ATTRIBUTE, (Context) GroundhogService.this);
    ...
    public void configureRealm() throws IOException {
        File realmProps = new File(Groundhog.__JETTY_DIR + "/" + Groundhog.__ETC_DIR + "/realm.properties");
        if (realmProps.exists()) {
            HashLoginService realm = new HashLoginService("Console", Groundhog.__JETTY_DIR + "/" + Groundhog.__ETC_DIR + "/realm.properties");
    ...
    protected void startJetty() throws Exception {
        ...
        //Set jetty.home
        System.setProperty("jetty.home", Groundhog.__JETTY_DIR.getAbsolutePath());
